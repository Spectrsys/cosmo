...
public class EntityConverterTest extends TestCase {
    protected String baseDir = "/entityconverter/";
...
//        Assert.assertTrue(masterEvent.getStartDate().equals(testStart));
    ...
    protected Calendar getCalendar(String name) throws Exception {
        ...
        InputStream stream = getClass().getResourceAsStream(baseDir + name);
        if (stream == null) {
            throw new IllegalArgumentException("Resource " + baseDir + name + " not found");
        }
        Calendar calendar = cb.build(stream);
        return calendar;
    }
